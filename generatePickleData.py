...
def renderSeperateFrameAndSaveImage(data_reader, renderIndex, triPlane, planeBdInsRec, svPath, func_integration, funcLinParamCal, func_IntersectLine, funcLineAff, func_computeNewBbox, func_depthRender, func_lineRender, func_pointCheck, func_acceleratedFormPolygons):
    valCount = 1
    triPlaneVelo_tot_mulFrame = np.transpose(
        np.matmul(np.matmul(data_reader.tr_oxts2velo, data_reader.trs_grid2oxts[renderIndex]),
                  np.transpose(triPlane)))
    ...
    image = mpimg.imread(data_reader.rgbFilePaths[renderIndex])
    r = np.zeros_like(image[:, :, 0])
    g = np.zeros_like(image[:, :, 1])
    b = np.zeros_like(image[:, :, 2])
    r_gpu = cuda.mem_alloc(r.nbytes)
    g_gpu = cuda.mem_alloc(g.nbytes)
    b_gpu = cuda.mem_alloc(b.nbytes)
    depthMap = np.zeros_like(imgMask)
...
def renderAndGetwarppedImg(data_reader, renderIndex, triPlane, planeBdInsRec, svPath, func_integration, funcLinParamCal, func_IntersectLine, funcLineAff, func_computeNewBbox, func_depthRender, func_lineRender, func_pointCheck, func_acceleratedFormPolygons):
    valCount = 1
    triPlaneVelo_tot_mulFrame = np.transpose(
        np.matmul(np.matmul(data_reader.tr_oxts2velo, data_reader.trs_grid2oxts[renderIndex]),
                  np.transpose(triPlane)))
...
def samplePolygon(bdComp, sampleDense):
    ...
    transition = bdComp.transition * 0
    bt_center = np.mean(botPolygon, 0)
    num_botPts = np.size(botPolygon, 0)
    bt_rotM = np.eye(3)
...
class GPURender:
    ...
    def renderSpecificBd_seperateFrameChannel(self, data_reader, bdInd):
    ...
    def renderSpecificBd_BuildingPlane(self, data_reader, bdInd):
        bdComp = data_reader.buildingComp[bdInd]
        valCount = 0
        maxDepthDist = 130
        svPath = os.path.join('/media/shengjie/other/KITTI_scene_understanding/python_code/BuildingLocalization', 'trainingDataVisualization')
...
class tt_struct:
    ...
    def generatePickle(self):
        ...
        for seq in self.allSeq:
            ...
            for bdCompIdx in tmpDataReader.buildingComp:
                curBd = self.gpurender.renderSpecificBd(tmpDataReader, bdCompIdx)
                if len(curBd.rgbs_dict) > 0:
                    pickle.dump(curBd, open(
                        os.path.join(seqSvPath, str(bdCompIdx) + ".p"),"wb"))
                    print("%dth Bd finished" % bdCompIdx)

    def generatePickle_rgb(self):
        pathPrefix = '/media/shengjie/other/KITTI_scene_understanding/python_code/BuildingLocalization/trainingData_proofView'
        for seq in self.allSeq:
            seqSvPath = os.path.join(pathPrefix, seq)
            try:
                os.mkdir(seqSvPath)
            ...
            for bdCompIdx in tmpDataReader.buildingComp:
                curBd = self.gpurender.renderSpecificBd(tmpDataReader, bdCompIdx)
                if len(curBd.rgbs_dict) > 0:
                    for idx, frameIdx in enumerate(list(curBd.rgbs_dict.keys())):
                        img = curBd.rgbs_dict[frameIdx]
                        image = Image.fromarray(img.astype('uint8'), 'RGB')
                        image.save(os.path.join(seqSvPath, str(curBd.bdComp.id) + '_' + str(frameIdx)), "JPEG")
                        print("%dth Bd finished" % bdCompIdx)
    ...
    def generatePickle_seperateFrame(self):
        ...
        for seq in self.allSeq:
            ...
            tmpDataReader = self.dataprovider.getReader(seq)
            for bdCompIdx in tmpDataReader.buildingComp:
                curBd = self.gpurender.renderSeperateFrameAndSaveImage(tmpDataReader, bdCompIdx)
                if len(curBd.rgbs_dict) > 0:
                    pickle.dump(curBd, open(
                        os.path.join(seqSvPath, str(bdCompIdx) + ".p"),"wb"))
                    print("%dth Bd finished" % bdCompIdx)
    ...
    def generatePickle_specPlane(self):
        pathPrefix = '/media/shengjie/other/KITTI_scene_understanding/python_code/BuildingLocalization/trainingData_seperateFrame'
        for seq in self.allSeq:
            seqSvPath = os.path.join(pathPrefix, seq)
            try:
            ...
            tmpDataReader = self.dataprovider.getReader(seq)
            for bdCompIdx in tmpDataReader.buildingComp:
                curBd = self.gpurender.renderSpecificBd_BuildingPlane(tmpDataReader, bdCompIdx)
                if len(curBd.rgbs_dict) > 0:
                    pickle.dump(curBd, open(
                        os.path.join(seqSvPath, str(bdCompIdx) + ".p"),"wb"))
                    print("%dth Bd finished" % bdCompIdx)
