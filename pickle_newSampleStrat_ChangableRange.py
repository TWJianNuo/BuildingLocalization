...
class baselineModel:
    def __init__(self) -> object:
        ...
        self.scaleWeights = nn.Parameter(torch.FloatTensor([16, 16, 16]).cuda(), requires_grad=True)
        ...
        # self.weitghChecker = list(self.pre_imageNet.parameters()) + list(self.LSTM.parameters()) + list(
        #     self.paramPredictor.parameters()) + [self.scaleWeights]
        self.optimizerTrans = optim.SGD(list(self.pre_imageNet.parameters()) + list(self.LSTM.parameters()) + list(
            self.paramPredictor.parameters()) + [self.scaleWeights], lr=0.000005)
        self.optimizerVisibility = optim.SGD(self.visibilityPredictor.parameters(), lr=0.0000005)
    ...
    def forward(self, buildingEntity):
        ...
        if torch.sum(torch.isnan(lstmInput)) == 0:
            ...
            paramPrediction = (self.paramPredictor(lstmOutput[-1, :, :]) - 0.5).squeeze(0) * self.scaleWeights

            # paramSumRec = list()
            # for p in self.weitghChecker:
            #     paramSumRec.append(torch.sum(p).detach().cpu().numpy())
            # print(paramSumRec)
            print(self.scaleWeights)
        ...
        if paramPrediction is not None:
            ...
            for idx in buildingEntity.rgbs_dict.keys():
                ...
                if torch.sum(selector) > 0:
                    lossPt2d = lossPt2d + torch.sum(((a_2d - b_2d) * selector).pow(2)) / torch.sum(selector)
...
writer = SummaryWriter('/media/shengjie/other/KITTI_scene_understanding/python_code/BuildingLocalization/runs/logLoss2d_ex_fixedInit_changeRange_dNum')
