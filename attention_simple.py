...
class baselineModel:
    ...
    def attentionForward(self, buildingEntity):
        ...
        if torch.sum(torch.isnan(lstmInput)) == 0:
            ...
            integrated = torch.sum(lstmOutput, dim = 0)
    ...
    def initSv(self):
        torch.save({
            ...
            'paramPredictor_state_dict': self.paramPredictor.state_dict(),
            'visibilityPredictor_state_dict': self.visibilityPredictor.state_dict(),
        }, os.path.join('/media/shengjie/other/KITTI_scene_understanding/python_code/BuildingLocalization/initedModel', str(1)))

    def initLoad(self):
        checkpoint = torch.load(os.path.join('/media/shengjie/other/KITTI_scene_understanding/python_code/BuildingLocalization/initedModel', str(1)))
        self.pre_imageNet.load_state_dict(checkpoint['pre_imageNet_state_dict'])
        self.LSTM.load_state_dict(checkpoint['LSTM_state_dict'])
        self.paramPredictor.load_state_dict(checkpoint['paramPredictor_state_dict'])
        self.visibilityPredictor.load_state_dict(checkpoint['visibilityPredictor_state_dict'])
...
"""
# bsm.initSv()
writer = SummaryWriter('/media/shengjie/other/KITTI_scene_understanding/python_code/BuildingLocalization/runs/attention_logLoss2d_ex_fixedInit_simple')
