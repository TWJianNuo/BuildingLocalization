...
class pickleReader():
    ...
    def seqValidEntry(self):
        ...
        return seqList
    def seqAllEntry(self):
        seqList = list()
        for seq in self.seqNameSet:
...
for add in pkr.seqAllEntry():
    with open(add, "rb") as input:
        ...
        if torch.sum(torch.isnan(lossVal)) > 0:
            a = 1
        print("loss is %f" % lossVal)
"""

for i in range(iterationTime):
    # bdCompCom = pkr.randomProvideEntry()
    bdCompCom = pkr.randomValidEntry()
    vds = videoSequence(bdCompCom.rgbs_dict, bdCompCom.depths_dict, bdCompCom.bdComp.transition, bdCompCom.bdComp.visibility, np.sum(bdCompCom.bdComp.visibility) > 0)
    lossVal = bsm.train(vds)
    ...
    if i % 100 == 99:
        ...
        if np.mean(testLossVals) > 0:
            writer.add_scalar('TestLoss', np.mean(testLossVals), int(i / 100))
    if i % 1000 == 999:
        bsm.sv(i)