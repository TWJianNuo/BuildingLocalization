...
class baselineModel:
    def __init__(self) -> object:
        ...
        self.attentionWeight = nn.Sequential(nn.Linear(self.lstm_hidden, self.lstm_hidden), nn.ReLU(),
                                             nn.Linear(self.lstm_hidden, 1)).cuda()
        for m in self.attentionWeight:
            if isinstance(m, nn.Linear):
                nn.init.xavier_uniform_(m.weight)
        self.attentionSoftMax = nn.Softmax(dim=0)
        self.optimizerTrans = optim.SGD(list(self.pre_imageNet.parameters()) + list(self.LSTM.parameters()) + list(
            self.paramPredictor.parameters()) + list(self.attentionWeight.parameters()), lr=0.000001)
        """
        if keyword == 'baseLine':
            self.optimizerTrans = optim.SGD(list(self.pre_imageNet.parameters()) + list(self.LSTM.parameters()) + list(
                self.paramPredictor.parameters()), lr=0.01)
        elif keyword == 'loss3d':
        ...
        elif keyword == 'logLoss2d':
            self.optimizerTrans = optim.SGD(list(self.pre_imageNet.parameters()) + list(self.LSTM.parameters()) + list(
                self.paramPredictor.parameters()), lr=0.000001)
        elif keyword == 'xinitlogLoss2d':
            self.optimizerTrans = optim.SGD(list(self.pre_imageNet.parameters()) + list(self.LSTM.parameters()) + list(
                self.paramPredictor.parameters()), lr=0.000001)
        elif keyword == 'attention':
            self.attentionWeight = nn.Sequential(nn.Linear(self.lstm_hidden, self.lstm_hidden), nn.ReLU(),
                                                 nn.Linear(self.lstm_hidden, 1)).cuda()
            nn.init.xavier_uniform_(self.attentionWeight.parameters())
            self.attentionSoftMax = nn.Softmax(dim=0)
        """
    ...
    def attentionForward(self, buildingEntity):
        ...
        if torch.sum(torch.isnan(lstmInput)) == 0:
            ...
            attentionWeight = list()
            for i in range(lstmOutput.shape[0]):
                attentionWeight.append(self.attentionWeight(lstmOutput[i,:,:]))
            concatted = torch.cat(attentionWeight, dim=0)
            nomrmed = self.attentionSoftMax(concatted)
            integrated = torch.sum(lstmOutput * nomrmed.unsqueeze(2).repeat(1, 1, self.lstm_hidden), dim=0).squeeze(0)
            paramPrediction = (self.paramPredictor(integrated) - 0.5) * 16
            visiblityPrediction = self.visibilityPredictor(lstmOutput).squeeze(1)
            return paramPrediction, visiblityPrediction
        else:
            return None, None

    def org_forward(self, buildingEntity):
        imageTransforms = transforms.Compose([
            transforms.Normalize([0.485, 0.456, 0.406, 0.406], [0.229, 0.224, 0.225, 0.225])
        ])
        imgNum = len(buildingEntity.rgbs_dict)
    ...
    def trainLoss2d(self, buildingEntity):
        paramPrediction, visiblityPrediction = self.attentionForward(buildingEntity)
...
writer = SummaryWriter('/media/shengjie/other/KITTI_scene_understanding/python_code/BuildingLocalization/runs/attention_logLoss2d_ex_fixedInit')
