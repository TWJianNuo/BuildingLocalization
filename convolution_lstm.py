...
if __name__ == '__main__':
    ...
    # gradient check
    inputChan = 60
    convlstm = ConvLSTM(input_channels=inputChan, hidden_channels=[64], kernel_size=3, step=1, effective_step=[0]).cuda()
    loss_fn = torch.nn.MSELoss()

    input = Variable(torch.randn(4, inputChan, 64, 32)).cuda()
